    ...
    if event.reply_to_msg_id and not len(args) == 2:
        ...
        try:
            ...
            return await event.edit(
                "`Terjadi Kesalahan... Mohon Lapor Ke Grup` @YansenSupport", str(err)
            )
...
async def gben(userbot):
    ...
    if not sender.id == me.id:
        dark = await dc.reply("`Sebentar Tuan, Gban Orang Ini Lagi Di Proses!`")
    else:
        dark = await dc.edit("`Memproses Global Banned Pengguna Ini!!`")
    me = await userbot.client.get_me()
    await dark.edit(f"`Global Banned Aktif Ya Ngentod!!!`")
    ...
    if user:
        if user.id in DEVS:
            return await dark.edit(f"`Pembuat Gue Lo Gban? Ga Akan Bisa Bego 😜`")
        try:
        ...
        for i in testuserbot:
            try:
                ...
                a += 1
                await dark.edit(f"`Global Banned Anak Haram Berhasil✅`")
            except BaseException:
                b += 1
    else:
    ...
    try:
        if gmute(user.id) is False:
            return await dark.edit(f"**Orang Ini Udah Lo Gban Dongo**")
    except BaseException:
        pass
...
async def gunben(userbot):
    ...
    me = await dc.client.get_me()
    if not sender.id == me.id:
        dark = await dc.reply("`Ngelepas Gban Si Kontol Di Proses🚀`")
    else:
        dark = await dc.edit("`Tunggu Sebentar Nyet`")
    me = await userbot.client.get_me()
    await dark.edit(
        f"`Global Banned Di Lepas, Jangan Jadi Kacung Tele Lagi Ye Mahkluk Haram!!!`"
    )
    my_mention = "[{}](tg://user?id={})".format(me.first_name, me.id)
    f"@{me.username}" if me.username else my_mention
    await userbot.get_chat()
